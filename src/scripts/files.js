    ...
    _open: {
        v1p0: function (contents) {
            ...
            for (entry in lexicon) {
                lexicon[entry].push(false, []);
            } // assume pronunciation regularity, zero tags
        ...
        v1p5: function (contents) {
            ...
            try {
                ...
                window.alert(
                    'There was a problem loading the alphabetical order. Please contact the developer.'
                );
            }
            try {
                let txt = '';
                ...
                window.alert(
                    'There was a problem loading the romanizations. Please contact the developer.'
                );
            }
            try {
                onset_input.value = contents.Phonotactics.Initial.join(' ');
                ...
                window.alert(
                    'There was a problem loading the phonotactics data. Please contact the developer.'
                );
            }
            try {
                write_tables(contents.Tables);
                ...
                window.alert(
                    'There was a problem loading the tables data. Please contact the developer.'
                );
            }
            try {
                header_tags_text.value = contents.HeaderTags;
            } catch (err) {
            ...
            try {
                ...
                window.alert(
                    "The save file's data successfully was loaded, but an error occurred while parsing it. Please contact the developer."
                );
            }
            tab_btns[0].onclick(); // Return to Lexicon tab
        },
        v1p6: function (contents) {
            ...
            try {
                ...
                window.alert(
                    'There was a problem loading the phrasebook. Please contact the developer.'
                );
            }
            try {
                onset_input.value = contents.Phonotactics.Initial.join(' ');
            ...
            try {
                ...
                window.alert(
                    "The save file's lexicon data successfully was loaded, but an error occurred while parsing it. Please contact the developer."
                );
            }
            try {
                update_categories();
                ...
                window.alert(
                    "The save file's phrasebook data successfully was loaded, but an error occurred while parsing it. Please contact the developer."
                );
            }
            tab_btns[0].onclick(); // Return to Lexicon tab
        },
        v1p7: function (contents) {
            ...
            try {
                document.getElementById('ignore-diacritic').checked =
                    contents.IgnoreDiacritics;
            } catch (err) {
                console.log(err);
            }
            try {
        ...
        v1p8: function (contents) {
            ...
            try {
                ...
                window.alert(
                    'There was a problem loading the documentation data. Please contact the developer.'
                );
                console.log(err);
            }
            try {
    ...
    _export: {
        txt: async function () {
            ...
            let file_handle = await window.showSaveFilePicker({
                suggestedName: `${file_name_input.value}.txt`,
            });
        ...
        csv: async function () {
            ...
            let file_handle = await window.showSaveFilePicker({
                suggestedName: `${file_name_input.value}.csv`,
            });
            await file_handle.requestPermission({ mode: 'readwrite' });
            let file = await file_handle.createWritable();
            try {
        ...
        json: async function () {
            ...
            let file_handle = await window.showSaveFilePicker({
                suggestedName: `${file_name_input.value}.json`,
            });
            await file_handle.requestPermission({ mode: 'readwrite' });
            let file = await file_handle.createWritable();
            try {
        ...
        html: {
            all: async function () {
                ...
                scripts.innerHTML = `
                    ...
                    function sort_lex_keys() {
                        let htags = ${JSON.stringify(
                            header_tags_text.value.toLowerCase().trim().split(/\\s+/)
                        )};
                ...
                let file_handle = await window.showSaveFilePicker({
                    suggestedName: `${file_name_input.value}.html`,
                });
                await file_handle.requestPermission({ mode: 'readwrite' });
                let file = await file_handle.createWritable();
                try {
            ...
            docs: async function () {
                ...
                head.innerHTML = `
                    <meta charset="UTF-8" />
                    <title>${file_name_input.value} Docs</title>
                    <link rel="preconnect" href="https://fonts.googleapis.com">
                    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
                    <link href="https://fonts.googleapis.com/css2?family=Gentium+Book+Plus:ital,wght@0,400;0,700;1,400;1,700&display=swap" rel="stylesheet">
                `;
                ...
                let file_handle = await window.showSaveFilePicker({
                    suggestedName: `${file_name_input.value}_Docs.html`,
                });
                await file_handle.requestPermission({ mode: 'readwrite' });
                let file = await file_handle.createWritable();
                try {
